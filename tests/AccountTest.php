...
test('account error resumable', function () {
    expect(Account::accountErrorResumable('429 too many requests'))
        ->toBeTrue()
        ->and(Account::accountErrorResumable('quota exceeded'))
        ->toBeTrue()
        ->and(Account::accountErrorResumable('disabled claude subscription access'))
        ->toBeFalse()
        ->and(Account::accountErrorResumable('random error'))
        ->toBeFalse();
});
...
test('prefer account error text', function () {
    ...
    expect(Account::preferAccountErrorText($transient, $permanent))
        ->toBe($permanent)
        ->and(Account::preferAccountErrorText($permanent, $transient))
        ->toBe($permanent);
});
...
test('prefer rate limit reset', function () {
    ...
    expect(Account::preferRateLimitReset($r1, $r2))->toBe($r2)->and(Account::preferRateLimitReset($r2, $r1))->toBe($r2);
});
